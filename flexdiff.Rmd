...
Below I create an object named `deltas` that will store the results of the `diffdf` call on the two data frames I created. 
...
## Ex1 - Background colors
Change the background color for cells of interest.
```{r}
df2 %>% 
  flextable() %>%
...
## Ex2 - Cell Borders
Adding a border around cells of interest. Note: This requires the `officer` package. 
...
As part of my current responsibilities, I receieve statistical tables saved as data sets (e.g. .sas7bdat, .csv) that I process in R. Given that this work flow is well established, I looked at whether comparisons between statistical tables were feasible with the packages outlined in this tutorial. One of the primary aims would be to detect changes between versions. If so, another aim would be to develop this into an application for ease of use by non-technical staff. I want to thank my colleague Robert (R.A.) for this suggestion!
...
* I have a hardcoded a rule to highlight in yellow the prescence of `-9999` in the `P` column. 
  + You'll also notice the background for this cell is blue, because this cell value is also different from the original table.
